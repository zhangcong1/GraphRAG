 ...
 * 知识图谱向量搜索命令处理器
 */
async function searchKnowledgeGraphCommand() {
    ...
    try {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const searchResults = await vectorizer.searchSimilarNodes(query, selectedCollection, { topK, threshold });
                ...
                // 5. 转换为快速选择项
                const quickPickItems = searchResults.map((result, index) => {
...
async function handleSearchResultSelection(item) {
    try {
        ...
        const action = await vscode.window.showInformationMessage(detailMessage, ...actions);
        switch (action) {
            case '打开文件':
                if (item.filePath) {
 ...
 * 在指定位置打开文件
 */
async function openFileAtLocation(filePath, startLine, endLine) {
    try {
 ...
 * 显示完整内容
 */
async function showFullContent(item) {
    try {
        // 创建一个临时文档显示完整内容
        const content = [
            `# 搜索结果详情`,
 ...
 * 批量搜索命令（支持多个查询）
 */
async function batchSearchKnowledgeGraphCommand() {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        vscode.window.showErrorMessage('请先打开一个工作区');
        return;
...
//# sourceMappingURL=searchKnowledgeGraph.js.map