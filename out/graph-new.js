...
class GraphBuilder {
     ...
     * 构建标准知识图谱JSON结构
     */
    buildStandardGraph(entities, fileImports, fileExports) {
        // 清空之前的数据
        this.nodes.clear();
        this.edges.clear();
        // 构建标准JSON节点数组
     ...
     * 构建传统格式（向后兼容）
     */
    buildLegacyNodesAndEdges(entities, fileImports, fileExports) {
        // 1. 创建文件和目录节点
     ...
     * 保存标准知识图谱JSON到文件
     */
    saveStandardGraphToFile(standardJson) {
        try {
     ...
     * 创建标准文件和目录节点
     */
    createStandardFileAndDirectoryNodes(entities) {
     ...
     * 创建目录层次结构
     */
    createStandardDirectoryHierarchy(filePath, directoryNodes, directorySet) {
        const relativePath = path.relative(this.workspacePath, filePath);
     ...
     * 创建标准代码元素节点
     */
    createStandardEntityNodes(entities) {
        const entityNodes = [];
        for (const entity of entities) {
     ...
     * 创建包含关系
     */
    createStandardContainsRelationships(entities, projectId, directoryNodes, fileNodes, entityNodes) {
        const edges = [];
        let edgeId = 1;
        // 项目包含目录
     ...
     * 创建定义关系
     */
    createStandardDefinedInRelationships(entityNodes, fileNodes) {
        const edges = [];
        let edgeId = 1000;
        for (const entity of entityNodes) {
            const file = fileNodes.find(f => f.absolute_path === entity.absolute_path);
     ...
     * 创建导入/导出关系
     */
    createStandardImportExportRelationships(fileImports, fileExports, fileNodes) {
        const edges = [];
        let edgeId = 2000;
        // 处理导入关系
     ...
     * 创建调用关系
     */
    createStandardCallRelationships(entities, entityNodes) {
        const edges = [];
        let edgeId = 3000;
        // 简化实现：基于代码片段中的函数调用模式
                ...
                for (const calledName of calledFunctions) {
                    ...
                    for (const targetFunc of targetFunctions) {
                        if (targetFunc.file_path !== entity.file_path || targetFunc.name !== entity.name) {
                            const sourceNode = entityNodes.find(n => n.absolute_path === entity.file_path &&
                                n.name === entity.name &&
                                n.start_line === entity.start_line);
                            const targetNode = entityNodes.find(n => n.absolute_path === targetFunc.file_path &&
     ...
     * 创建语义关系
     */
    createStandardSemanticRelationships(entityNodes) {
        const edges = [];
        let edgeId = 4000;
        // 基于语义标签创建关系
     ...
     * 检测社区
     */
    detectStandardCommunities(nodes, edges) {
        const communities = [];
        // 基于文件路径的社区检测
     ...
     * 创建元数据
     */
    createStandardMetadata(entities, nodes, edges) {
     ...
     * 判断是否应该创建语义关系
     */
    shouldCreateSemanticRelation(entityA, entityB) {
        // 如果是同一类型的实体，且有共同标签
     ...
     * 创建跨文件的语义关系（限制数量）
     */
    createCrossFileSemanticRelationships(entities) {
        // 只在有导入关系的文件之间创建关系
        const importRelatedFiles = new Set();
     ...
     * 计算社区分数
     */
    calculateCommunityScoreOld(nodeIds) {
        let internalEdges = 0;
        let externalEdges = 0;
        for (const edge of this.edges.values()) {
     ...
     * 提取社区标签
     */
    extractCommunityTagsOld(nodeIds) {
    ...
    resolveImportPath(workspacePath, fromFile, importPath) {
    ...
    generateFileSemanticTags(language, elementTypes, fileName) {
        const tags = ['file'];
        if (language)
            tags.push(language);
    ...
    generateDirectorySemanticTags(dirName, fullPath) {
    ...
    calculateCommunityScore(nodes, edges) {
    ...
    inferFunctionality(dirPath, tags) {
        const functionality = [];
    ...
    calculateParsingStatistics(entities, totalFiles) {
        const stats = {
...
//# sourceMappingURL=graph-new.js.map