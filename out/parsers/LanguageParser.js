...
class LanguageParser {
    ...
    getLanguageConfig(language) {
        ...
        if (!parser)
            return null;
        const nodeTypeMap = {
            'python': types_1.SPLITTABLE_NODE_TYPES.python,
            'py': types_1.SPLITTABLE_NODE_TYPES.python,
            'java': types_1.SPLITTABLE_NODE_TYPES.java,
            'go': types_1.SPLITTABLE_NODE_TYPES.go,
            'rust': types_1.SPLITTABLE_NODE_TYPES.rust,
            'rs': types_1.SPLITTABLE_NODE_TYPES.rust
        };
        return {
            parser,
    ...
    // 语言特定的正则解析方法
    parsePythonWithRegex(filePath, fileName, content, result) {
        // 解析Python函数
    ...
    parseJavaWithRegex(filePath, fileName, content, result) {
        // 解析Java方法
    ...
    parseGoWithRegex(filePath, fileName, content, result) {
        // 解析Go函数
    ...
    parseRustWithRegex(filePath, fileName, content, result) {
        // 解析Rust函数
