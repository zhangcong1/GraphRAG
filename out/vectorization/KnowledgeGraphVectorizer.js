 ...
 * 知识图谱向量化器类
 */
class KnowledgeGraphVectorizer {
    embeddingService;
    vectorDB;
    config;
     ...
     */
    async vectorizeKnowledgeGraph(nodes, collectionName = 'knowledge_graph', progressCallback) {
        ...
        try {
            ...
            const embeddings = await this.embeddingService.getEmbeddingsWithProgress(validTexts, (progress, total) => {
     ...
     * 创建向量文档
     */
    createVectorDocument(node, embedding, textContent) {
        const metadata = {
            nodeId: node.id,
     ...
     * @param options 搜索选项
     */
    async searchSimilarNodes(queryText, collectionName = 'knowledge_graph', options = {}) {
        try {
            console.log(`🔍 搜索相似节点: "${queryText}"`);
            // 获取查询文本的向量
            const queryEmbeddings = await this.embeddingService.getEmbeddings([queryText], this.config.useSimulation);
     ...
     * @param collectionName 集合名称
     */
    async vectorizeFileNodes(nodes, filePaths, collectionName = 'knowledge_graph') {
        // 过滤指定文件的节点
...
//# sourceMappingURL=KnowledgeGraphVectorizer.js.map