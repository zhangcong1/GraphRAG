...
const chromeMilvusAdapter_1 = require("./milvus/chromeMilvusAdapter");
const milvusConfig_1 = require("./config/milvusConfig");
const indexedRepoManager_1 = require("./storage/indexedRepoManager");
...
class EmbeddingModel {
    ...
    static async getConfig() {
        if (!this.config) {
            const config = await milvusConfig_1.MilvusConfigManager.getOpenAIConfig();
            if (!config) {
    ...
    static async embedSingle(text) {
        const results = await this.embedBatch([text]);
        return results[0];
    }
    static async getInstance(_progress_callback = undefined) {
...
class MilvusVectorDB {
    ...
    constructor(repoId) {
        ...
        this.adapter = new chromeMilvusAdapter_1.ChromeMilvusAdapter(this.repoCollectionName);
    }
    async initialize() {
        try {
    ...
    async searchSimilar(queryVector, limit = 20) {
        try {
...
function splitCode(content, language = '', chunkSize = 1000, chunkOverlap = 200) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentSize + lineSize > chunkSize && currentChunk.length > 0) {
            ...
            const overlapLines = Math.min(Math.floor(chunkOverlap / (chunkContent.length / currentChunk.length)), currentChunk.length);
...
async function handleTestMilvusConnection(sendResponse) {
    try {
        console.log('Testing Milvus connection...');
        const adapter = new chromeMilvusAdapter_1.ChromeMilvusAdapter('test_connection');
...
async function handleIndexRepo(request, sendResponse) {
    try {
        ...
        await indexedRepoManager_1.IndexedRepoManager.addIndexedRepo({
            id: repoId,
            owner,
            repo,
...
async function processFileList(files, owner, repo, repoId, vectorDB, chunkSize, chunkOverlap) {
...
async function processChunkBuffer(chunkBuffer, vectorDB) {
    if (chunkBuffer.length === 0)
        return;
    // Extract chunks
...
async function processChunkBatch(chunks, vectorDB) {
    // Generate embedding vectors using batch processing
    const chunkContents = chunks.map(chunk => chunk.content);
    const embeddings = await EmbeddingModel.embedBatch(chunkContents);
...
async function handleSearchCode(request, sendResponse) {
    try {
        ...
        await indexedRepoManager_1.IndexedRepoManager.updateLastSearchTime(repoId);
...
async function handleClearIndex(request, sendResponse) {
    try {
        const { owner, repo } = request;
        const repoId = `${owner}/${repo}`;
        const vectorDB = new MilvusVectorDB(repoId);
        ...
        await indexedRepoManager_1.IndexedRepoManager.removeIndexedRepo(repoId);
...
async function handleCheckIndexStatus(request, sendResponse) {
    try {
        const { owner, repo } = request;
        const repoId = `${owner}/${repo}`;
        const indexedRepo = await indexedRepoManager_1.IndexedRepoManager.isRepoIndexed(repoId);
        if (indexedRepo) {
            try {
                const vectorDB = new MilvusVectorDB(repoId);
                await vectorDB.initialize();
...
async function handleGetIndexedRepos(sendResponse) {
    try {
        const repos = await indexedRepoManager_1.IndexedRepoManager.getRecentlyIndexedRepos(20);
        sendResponse({
            success: true,
            repos
        });
