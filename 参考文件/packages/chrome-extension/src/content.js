...
function injectUI() {
    ...
    const repoNav = document.querySelector('nav.UnderlineNav');
    const existingContainer = document.getElementById('code-search-container');
...
function startIndexing() {
    ...
    const indexBtn = document.getElementById('index-repo-btn');
...
async function checkIndexStatus() {
    ...
    const statusEl = document.getElementById('indexing-status');
    if (statusEl)
        statusEl.textContent = 'Checking repository index status...';
    try {
...
function updateUIState(isIndexed, indexInfo) {
    const indexBtn = document.getElementById('index-repo-btn');
    const clearBtn = document.getElementById('clear-index-btn');
    const searchBtn = document.getElementById('search-btn');
    const searchInput = document.getElementById('search-input');
...
function showRecentRepos() {
    ...
    if (!recentReposContainer || !recentReposList)
        return;
...
function displayRecentRepos(repos) {
    ...
    list.querySelectorAll('.go-to-repo-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const url = e.target.getAttribute('data-repo-url');
            if (url) {
