...
function saveOptions() {
    ...
    const openaiInput = document.getElementById('openai-token');
    // Milvus configuration inputs
    const milvusAddressInput = document.getElementById('milvus-address');
    const milvusTokenInput = document.getElementById('milvus-token');
    const milvusDatabaseInput = document.getElementById('milvus-database');
    if (tokenInput) {
...
function restoreOptions() {
    ...
    chrome.storage.sync.get({
        ...
        if (openaiTokenInput)
            openaiTokenInput.value = items.openaiToken || '';
        if (milvusAddressInput)
            milvusAddressInput.value = items.milvusAddress || '';
        if (milvusTokenInput)
            milvusTokenInput.value = items.milvusToken || '';
        if (milvusDatabaseInput)
            milvusDatabaseInput.value = items.milvusDatabase || 'default';
    });
}
function testMilvusConnection() {
    ...
    const testButton = document.getElementById('test-milvus');
    if (resultSpan) {
