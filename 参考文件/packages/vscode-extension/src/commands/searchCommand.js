...
class SearchCommand {
    ...
    async execute(preSelectedText) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                let results = await this.context.semanticSearch(codebasePath, query.term, query.limit || 20, 0.3, // similarity threshold
                filterExpr);
     ...
     */
    async executeForWebview(searchTerm, limit = 50, fileExtensions = []) {
        // Get workspace root for codebase path
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders || workspaceFolders.length === 0) {
        ...
        let results = await this.context.semanticSearch(codebasePath, searchTerm, limit, 0.3, // similarity threshold
        filterExpr);
        return results;
    }
    /**
     ...
     * Generate quick pick items for VS Code
     */
    generateQuickPickItems(results, searchTerm, workspaceRoot) {
